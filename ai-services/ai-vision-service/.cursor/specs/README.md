...
Coloque aqui os `.md` de especificação do **vision-platform**. O agente deve lê-los antes de implementar qualquer componente.
...
O **inference-engine** é **Mojo-only**. Specs descrevem comportamento e contrato, não pedem C++/CUDA.
`model-training` permanece Python, gerenciado com **pixi**.
